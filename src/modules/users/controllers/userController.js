...
exports.updateMyPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (!isMatch) {
            if (isJson) return res.status(400).json({ status: 'error', message: 'Mật khẩu hiện tại không chính xác!' });
            req.flash('error_msg', 'Mật khẩu hiện tại không chính xác!');
            return res.redirect('back');
        }

        if (newPassword !== confirmPassword) {
            if (isJson) return res.status(400).json({ status: 'error', message: 'Xác nhận mật khẩu mới không khớp!' });
            req.flash('error_msg', 'Xác nhận mật khẩu mới không khớp!');
            return res.redirect('back');
        }

        if (newPassword.length < 6) {
            if (isJson) return res.status(400).json({ status: 'error', message: 'Mật khẩu phải có ít nhất 6 ký tự!' });
        ...
        if (isJson) return res.status(500).json({ status: 'error', message: 'Lỗi hệ thống: ' + err.message });
        next(err);
    }
};
